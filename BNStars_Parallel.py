...
We use the multiprocessing library of Python to solve boson, neutron or mixed boson-neutron stars
...
if __name__ == '__main__':
    ...
    pmax = 0.9*max(data[0])
    pmin = 1.1*min(data[0])
    pmx = pmax*u.MeV_fm3_2_Msun_km3
    pmn = pmin*u.MeV_fm3_2_Msun_km3
    ...
    ip = 2.0
    iphi = 1.5
    iw = 2**abs(iLambda/50 + iKappa/10)*12**iphi # Estimation of the bosons internal frequency
    ...
    def BS(phi0):
        ...
        r = np.arange(R0, Rf, step)
        N = len(r)
        iw_dyn = 2**abs(iLambda/50 + iKappa/10)*12**iphi_dyn
    ...
    def BNS(phi0, p0):
        ...
        if iphi_dyn == 0:
            ...
            iw_dyn = 2**abs(iLambda/50 + iKappa/10)*(14*(np.floor(ip_dyn) + 1)**(abs(np.log(iphi_dyn))))**iphi_dyn
        r = np.arange(R0, Rf, step)
        N = len(r)
    ...
    with mp.Pool(processes = 12) as pool:
        for sol in pool.map(solve_BS, phi0):
    ...
    a0_BS = np.zeros(N_BS)  
    for i in range(N_BS):
            R_BS[i] = solutions[i][1][0]
            M_BS[i] = solutions[i][1][1]
            B_BS[i] = solutions[i][1][2]
            omega_BS[i] = solutions[i][1][3]
            a0_BS[i] = solutions[i][1][4]

    np.savetxt('BS.dat', np.array([R_BS, M_BS, B_BS, phi0, omega_BS, a0_BS]).T)
    '''

    # Solve many NS in parallel
    ...
    def solve_NS(p0):
        try:
            ...
            print('Not achieved NS')
            return [np.zeros(5), np.zeros(4)]
    
    solutions = []
    with mp.Pool(processes = 12) as pool:
        for sol in pool.map(solve_NS, p0):
            solutions.append(sol)
    ...
    a0_NS = np.zeros(N_NS)
    for i in range(N_NS):
        R_NS[i] = solutions[i][1][0]
        M_NS[i] = solutions[i][1][1]
        B_NS[i] = solutions[i][1][2] 
        a0_NS[i] = solutions[i][1][3]

    np.savetxt('NS.dat', np.array([R_NS, M_NS, B_NS, p0, a0_NS]).T)
   
    '''
    # Solve many BNS in parallel
    phi0 = scalephi*np.exp(np.linspace(np.log(0.1), np.log(2.0), 50))
    p0 = scalep*np.exp(np.linspace(np.log(8e-3), np.log(5.0), 50))
    ...
    paramlist = list(itertools.product(p0, phi0))
    
    def solve_BNS(params):
        p0, phi0 = params
        try:
            sol = BNS(phi0, p0)
            return sol
    ...
    solutions = []
    with mp.Pool(processes = 12) as pool:
        for sol in pool.map(solve_BNS, paramlist):
            solutions.append(sol)
    ...
    for i in range(N_NS*N_BS):
        R_NBS[i] = solutions[i][1][0]
        M_NBS[i] = solutions[i][1][1]
        B_NBS[i] = solutions[i][1][2]
        R_BNS[i] = solutions[i][1][3]
        M_BNS[i] = solutions[i][1][4]
        B_BNS[i] = solutions[i][1][5]
        omega_BNS[i] = solutions[i][1][6]
        a0_BNS[i] = solutions[i][1][7]

    np.savetxt('BNS.dat', np.array([R_NBS, M_NBS, B_BNS, R_BNS, M_BNS, B_BNS, p0_BNS.ravel(), phi0_BNS.ravel(), omega_BNS, a0_BNS]).T)
    '''
